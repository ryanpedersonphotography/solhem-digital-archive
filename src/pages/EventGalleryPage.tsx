...
export default function EventGalleryPage() {
  ...
  const { getPhotoTags, getTagStats } = useTagStore();
  ...
  }, [event, getPhotoRating, getPhotoTags]);
  
  // Filter by tags
  const filteredPhotos = useMemo(() => {
    if (selectedTag === 'all') return photosWithMeta;
    return photosWithMeta.filter(p => p.tags?.includes(selectedTag));
  }, [photosWithMeta, selectedTag]);
            ...
            >
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-gray-900">
                  {photoMeta.photo.caption || `Photo ${index + 1}`}
                </h3>
